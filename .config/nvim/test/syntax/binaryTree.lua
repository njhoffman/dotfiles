...
function Node:data() return self._data end
...
local function contains(node, data)
  ...
  elseif data < node:data() then
    ...
    if node.left ~= nil then return contains(node.left, data) end
  else
    if node.right ~= nil then return contains(node.right, data) end
  end
