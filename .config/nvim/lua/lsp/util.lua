...
function U.hover()
  ...
  if current_hovered_word ~= new_current_hovered_word then lsp.buf.hover() end
...
function U.apply_globals(opts) for k, v in pairs(opts) do g[k] = v end end
...
function U.move_cursor_from_tree()
  ...
  if string.find(buf, "NERD_tree") and nr > 1 then cmd("wincmd l") end
end
...
function U.help_tab()
  if bo.buftype == "help" then
    cmd("wincmd L")
    local nr = api.nvim_get_current_buf()
    api.nvim_buf_set_keymap(nr, "", "q", ":q<CR>",
                            {noremap = true, silent = true})
  end
end

function U.esc(arg) return api.nvim_replace_termcodes(arg, true, false, true) end
...
function U.hi(group, styles)
  ...
  local command = string.format("hi %s", group)
  if styles.bg then command = string.format("%s guibg=%s", command, styles.bg) end
  if styles.fg then command = string.format("%s guifg=%s", command, styles.fg) end
  if styles.cbg then
  ...
  if styles.gui then command = string.format("%s gui=%s", command, styles.gui) end
  cmd(command)
end
...
function U.highlights(hi_table)
  for group, styles in pairs(hi_table) do U.hi(group, styles) end
end

function U.hiLink(src, dest) cmd(string.format("hi! link %s %s", src, dest)) end

function U.hiLinks(hi_table)
  for src, dest in pairs(hi_table) do U.hiLink(src, dest) end
end
...
function _G.console_log()
  ...
  cmd(string.format(
          "keepjumps norm!oconsole.log('%s ->', %s); // eslint-disable-line no-console",
          word, word))
  fn.winrestview(view)
end
...
local special_buffers = {
  "git", "undotree", "help", "startify", "vim-plug", "NvimTree"
}
...
function _G.is_special_buffer()
  ...
  for _, b in ipairs(special_buffers) do if filetype == b then return true end end
  return false
end
...
function _G.log(msg)
  ...
  require('core.utils').log_to_file(config.Opts.log_path)(vim.inspect(msg))
end
