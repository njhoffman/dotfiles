...
local replace_tabs = function(value)
  return fn.substitute(value, '\\t', fn['repeat'](' ', api.nvim_buf_get_option(
                                                      0, 'tabstop')), 'g')
end
...
local is_ignored = function()
  return
      fn.index(fold_exclusions, api.nvim_buf_get_option(0, 'filetype')) >= 0 or
          vim.wo.diff
end

local is_import = function(item)
  local sub = item:match('^import')
  if sub ~= nil then return #sub > 0 end
  return false
end
...
local render = function()
  ...
  local text_length = #fn.substitute(fold_start .. fold_end, '.', 'x', 'g') +
                          column_size
  return string.format('%s%s%s', fold_start, fn['repeat'](fold_char,
                                                          fn.winwidth(0) -
                                                              text_length - 10),
                       fold_end)
end
