...
if TOTALS:
    get = lambda cmd: subprocess.check_output(cmd).decode("utf-8").strip()
    all_count = len(get(["ps", "-aux"]).splitlines())
    user_count = len(get(["ps", "-ux"]).splitlines())
    totals_output += '%i:%i' % (user_count, all_count)

print(gui_output + process_output + totals_output)
